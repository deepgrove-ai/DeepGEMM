...
sm90_fp8_gemm_1d_rowwise_impl(float* sfb, int* grouped_layout,
                        uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
    ...
    // ref: const uint32_t& smem_sfb_size = align<uint32_t>(shape_k_scales * (kMustUseUniformedScaleB ? 1 : 2) * sizeof(float), sizeof(Barrier));
    const uint32_t& smem_sfb_size = align<uint32_t>((kMustUseUniformedScaleB ? 1 : 2) * sizeof(float), sizeof(Barrier));
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            // ref: uint32_t num_sfb = shape_k_scales * (num_former_iters >= num_full_iters ? 1 : 2);
            uint32_t num_sfb = (num_former_iters >= num_full_iters ? 1 : 2);
            ...
            if (threadIdx.x >= 32) {
                ...
                auto previous_group_offset = scheduler.get_global_idx<true>(shape_n_sfb, 0, 0, m_block_idx);
                auto local_sfb = sfb + previous_group_offset + ((n_block_idx * BLOCK_N) / BLOCK_K) * stride_n_sfb; // one float per n_sfb
                ...
                for (uint32_t i = threadIdx.x - 32; i < num_sfb; i += kNumMathThreads - 32)
                    ...
                    st_shared(smem_sfb + i, __ldg(i < shape_k_scales ? local_sfb : local_sfb + stride_n_sfb));
            }
            ...
            if (scheduler.is_computation_valid(m_block_idx, math_wg_idx * WGMMA::M)) {
                ...
                dispatch_num_former_iters<0, kGap, kEnd>(kShouldOptimize ? num_former_iters : 0, [&](auto _) {
                    ...
                    for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                        ...
                        // Read B scales
                        // this has been edited so that it is constant across k rather than stepping
                        // ref: float scale_b_0 = ld_shared(smem_sfb + k_block_idx), scale_b_1;
                        float scale_b_0 = ld_shared(smem_sfb), scale_b_1;
                        // NOTES: even some blocks do not need to read the second row, but we still load one to align with other blocks
                        // ok so the reasoning here is that we won't have 2d scales, so this is not necessary
                        if constexpr (not kMustUseUniformedScaleB)
                        ...
                        for (uint32_t local_idx = 0; local_idx < BLOCK_M / WAVE_BLOCK_M; ++ local_idx) {
                            ...
                            // again, cause we are constant across k
                            // if constexpr (not kMustUseUniformedScaleB)
