...
sm90_fp8_gemm_1d1d_rowwise_impl(__nv_fp8_e4m3* gmem_a_ptr, __nv_fp8_e4m3* gmem_b_ptr,
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        // [FIX] Use TiledMMA to map Layouts instead of manual pointer math
        // Define the TiledMMA. We assume standard SM90 64x128 shape (128 threads).
        using mma_op = cute::SM90_64x128x32_F8F8F32_SS;
        using TiledMma = cute::TiledMMA<
            mma_op,
            Layout<Shape<Int<1>, Int<1>, Int<1>>>,
            Layout<Shape<Int<1>, Int<1>, Int<1>>>
        >;
        TiledMma tiled_mma;
        auto thr_mma = tiled_mma.get_slice(threadIdx.x % 128); // 128 threads per WG
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            // Accumulators: Use raw float array but view it through TiledMMA layout
            float accum[WGMMA::kNumAccum] = {0};       
            float final_accum[WGMMA::kNumAccum] = {0}; 

            // Create Tensor Views for Accumulators
            auto tC_rAcc = make_tensor(make_rmem_ptr(accum), TiledMma::LayoutC_TV{});
            auto tC_rFinal = make_tensor(make_rmem_ptr(final_accum), TiledMma::LayoutC_TV{});

            // -----------------------------------------------------------
            // 1. Wait and Load Scales (Aligned to C)
            // -----------------------------------------------------------
            uint32_t scale_phase = (tile_idx & 1);
            scale_full_barrier->wait(scale_phase);
            
            // [FIX] Create Layouts for Scales and Partition by C
            // View SFA as (M, N) with stride (1, 0) -> broadcasting over N
            // View SFB as (M, N) with stride (0, 1) -> broadcasting over M
            // Note: We use math_wg_idx to point to the correct 64xN block if BLOCK_M > 64
            auto sSFA = make_tensor(make_smem_ptr(smem_sfa_ptr + math_wg_idx * 64), 
                                    make_layout(make_shape(Int<64>{}, Int<BLOCK_N>{}), make_stride(Int<1>{}, Int<0>{})));
            auto sSFB = make_tensor(make_smem_ptr(smem_sfb_ptr), 
                                    make_layout(make_shape(Int<64>{}, Int<BLOCK_N>{}), make_stride(Int<0>{}, Int<1>{})));

            // Partition into registers matching accumulators
            auto tSFA_r = make_fragment_like(tC_rAcc);
            auto tSFB_r = make_fragment_like(tC_rAcc);
            
            cute::copy(thr_mma.partition_C(sSFA), tSFA_r);
            cute::copy(thr_mma.partition_C(sSFB), tSFB_r);
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; ++ k_block_idx) {
                ...
                #pragma unroll
                for (uint32_t i = 0; i < WGMMA::kNumAccum; ++ i) warpgroup_fence_operand(accum[i]);
                warpgroup_arrive();
            ...
            // -----------------------------------------------------------
            // 3. Apply Scales & Store (Using CuTe Layouts)
            // -----------------------------------------------------------
            
            // [FIX] Apply scales using element-wise Tensor arithmetic
            // This works because tSFA_r/tSFB_r are partitioned exactly like tC_rFinal
            #pragma unroll
            for(int i = 0; i < size(tC_rFinal); ++i) {
                tC_rFinal(i) = tC_rFinal(i) * tSFA_r(i) * tSFB_r(i);
            }

            // Flush stores
            if (warp_idx % 4 == 0 and cute::elect_one_sync()) cute::tma_store_wait<0>();
            cutlass::arch::NamedBarrier::sync(128, math_wg_idx);

            // [FIX] Store to Shared Memory using CuTe Copy (Un-swizzles automatically)
            // View SMEM D as (64, BLOCK_N) for this warp group
            auto sD = make_tensor(make_smem_ptr(smem_d + math_wg_idx * 64 * BLOCK_N),
                                  make_layout(make_shape(Int<64>{}, Int<BLOCK_N>{}), LayoutRight{})); // Row-major

            auto tC_rD_partition = thr_mma.partition_C(sD);
            cute::copy(tC_rFinal, tC_rD_partition);
            ...
            if (warp_idx % 4 == 0 and cute::elect_one_sync()) {
                cute::SM90_TMA_REDUCE_ADD_2D::copy(
                    &tensor_map_cd, 
                    reinterpret_cast<const float2*>(smem_d + math_wg_idx * 64 * BLOCK_N), // Use specific WG slice
                    n_block_idx * BLOCK_N,
                    current_group_idx * shape_m + m_block_idx * BLOCK_M + math_wg_idx * 64);
